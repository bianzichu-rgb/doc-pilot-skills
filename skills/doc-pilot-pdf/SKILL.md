...
description: >
  ...
  needs to process a local PDF file before navigation. Handles: heading levels from font
  sizes (H1≥14pt, H2≥11pt), TOC/bookmark extraction, header/footer removal, dual-column
  reordering, garbage/CID text filtering, spec table reconstruction (Zipper algorithm),
  and figure position placeholders. Part of the doc-pilot skill suite.
...
hierarchy — not just raw text dump.
...
```bash
python ~/.claude/skills/doc-pilot-pdf/scripts/extract.py <pdf_path> [--output <md_path>] [--toc-only]
```
